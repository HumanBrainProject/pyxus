...
class TestSchemaRepository(TestCase):
 ...
 \
    expected_jsonld_schema = {'shapes': [
        {u'http://www.w3.org/ns/shacl#maxCount': [{u'@value': 1}], u'@id': u'_:ub1bL0C255',
         u'http://www.w3.org/ns/shacl#path': [{u'@id': u'http://xmlns.com/foaf/0.1/name'}],
         u'http://www.w3.org/ns/shacl#minCount': [{u'@value': 1}],
         u'http://www.w3.org/ns/shacl#datatype': [
             {u'@id': u'http://www.w3.org/2001/XMLSchema#string'}]},
        {u'http://www.w3.org/ns/shacl#property': [{u'@id': u'_:ub1bL0C255'}],
         ...
         u'@id': u'http://foaf_sh.com/PersonShape',
         u'@type': [u'http://www.w3.org/ns/shacl#NodeShape']}], '@context': {
        'shapes': {'@reverse': 'rdfs:isDefinedBy', '@type': '@id'},
        'owl': 'http://www.w3.org/2002/07/owl#',
        'isDefinedBy': {'@id': 'rdfs:isDefinedBy', '@type': '@id'},
        'rdfs': 'http://www.w3.org/2000/01/rdf-schema#'}, '@type': 'owl:Ontology'}
    json_schema_id = '_:ub1bL0C255'
    expected_jsonld_instance = [{u'http://xmlns.com/foaf/0.1/name': [{u'@value': u'Bar'}],
    ...
    def __ignore_id(self, value_to_ignore, deepdiff_dict):
        return value_to_ignore in deepdiff_dict['new_value'] or value_to_ignore in deepdiff_dict[
            'old_value']
    ...
    def test_create_turtle_schema(self):
        ...
        if res['values_changed']:
            assert all(self.__ignore_id(self.json_schema_id, value) for key, value in
                       res['values_changed'].iteritems())
        assert True
